package com.custodian.iar.handler;
...
public class CustIARHandler extends BaseWorkflowHandler<CustIAR> {
	
	CustIARLocalService custIARLocalService;
	
	@Reference(unbind = "-")
	public void setCustIARLocalService(CustIARLocalService custIARLocalService) {
		this.custIARLocalService = custIARLocalService;
	}
	...
	@Override
	public CustIAR updateStatus(int status, Map<String, Serializable> workflowContext) throws PortalException {
		...
		CustIAR custIAR = custIARLocalService.updateCustIARStatus(id, userId, status, serviceContext, WorkflowContextUtil.convert(workflowContext));

		return custIAR;
	}
	
	Log _log = LogFactoryUtil.getLog(CustIARHandler.class);
