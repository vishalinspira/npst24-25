<%@page import="com.stewardshipreport.constants.StewardshipReportPortletKeys"%>
<%@ include file="/init.jsp" %>

<portlet:resourceURL id="<%=StewardshipReportPortletKeys.stewardshipDocumentUpload%>" var="stewardPdfResourceURL" /> 
               ...
               <aui:form id="fileinfo" method="POST" >
                  ...
                  <div class="nps-input-box file-upload">
                     ...
                     <div class="file-select">
                        ...
                        <input type="file" id="chooseFile" name="stewardshipFile" accept=".pdf"/>
                     </div>
                     <label id="error-stewardshipFile" class="error-message text-danger"></label>
                  </div>
...
$(function(){
    ...
    $('#upload').on('click', function(){
    	if ($('input[name="stewardshipFile"]').get(0).files.length === 0) {
    	    console.log("No files selected.");
    	    $("#error-stewardshipFile").html("Please select a file to upload");
    	    return false;
    	}else if($('input[name="stewardshipFile"]').get(0).files[0].name != "Stewardship Report.pdf"){
    		console.log("No files selected.");
    	    $("#error-stewardshipFile").html("Please upload Stewardship Report.pdf");
    	    return false;
    	}
        ...
        $(".content").hide();
        $(".animaion").show();
        $("#error-stewardshipFile").html("");
        $.ajax({
            url: '${stewardPdfResourceURL}', 
            type: 'POST',
            data: fd,
