<%@page import="com.exceptionreporting.constants.ExceptionReportingPortletKeys"%>
<%@ include file="/init.jsp" %>

<portlet:resourceURL id="<%=ExceptionReportingPortletKeys.halfYearlyExceptionReportingDocumentUpload%>" var="exceptionReportingResourceURL" /> 
   ...
   <div class="row">
         <div class="col-12">
            <div class="text-right">
                 <p  class="back_btn"><a class="backbtn" href="/web/guest/half-yearly1"><i class="fas fa-arrow-left text-white pr-3"></i>Back</a></p>
            </div>
         </div>
     </div>
   ...
   <div class="row">
      <div class="col-lg-7 col-md-10 col-sm-12 col-12">
         <div class="nps-report">
            <div class="content">
               <h4>Half Yearly Exception Reporting</h4>
               <aui:form id="fileinfo" method="POST" >
                  ...
                  <div class="nps-input-box file-upload">
                     ...
                     <div class="file-select">
                        ...
                        <input type="file" id="chooseFile_1" name="exceptionReportPDFFile" accept=".pdf"/>
                     </div>
                     <label id="error-exceptionReportPDFFile" class="error-message text-danger"></label>
                  </div>
                  <div class="nps-input-box file-upload">
                     <label>Report </label>
                     <div class="file-select">
                        ...
                        <input type="file" id="chooseFile_2" name="exceptionReportDocFile" accept=".docx"/>
                     </div>
                     <label id="error-exceptionReportDocFile" class="error-message text-danger"></label>
                  </div>
...
$(function(){
    ...
    $('#upload').on('click', function(){
    	if ($('input[name="exceptionReportPDFFile"]').get(0).files.length === 0) {
    	    console.log("No files selected.");
    	    $("#error-exceptionReportPDFFile").html("Please select a file to upload");
    	    return false;
    	}else if($('input[name="exceptionReportPDFFile"]').get(0).files[0].name != "Half yearly Exception reporting.pdf"){
    		console.log("No files selected.");
    	    $("#error-exceptionReportPDFFile").html("Please upload Half yearly Exception reporting.pdf");
    	    return false;
    	}
    	
    	if ($('input[name="exceptionReportDocFile"]').get(0).files.length === 0) {
    	    console.log("No files selected.");
    	    $("#error-exceptionReportDocFile").html("Please select a file to upload");
    	    return false;
    	}else if($('input[name="exceptionReportDocFile"]').get(0).files[0].name != "Half yearly Exception reporting.docx"){
    		console.log("No files selected.");
    	    $("#error-exceptionReportDocFile").html("Please upload Half yearly Exception reporting.docx");
    	    return false;
    	}
        ...
        $(".content").hide();
        $(".animaion").show();
        $("#error-exceptionReportPDFFile").html("");
        $("#error-exceptionReportDocFile").html("");
        $.ajax({
            url: '${exceptionReportingResourceURL}', 
            type: 'POST',
            data: fd,
