...
<portlet:resourceURL id="/saveBenchmark/data" var="benchmarkFileURl"></portlet:resourceURL>
					...
					<input type="hidden" value="${reportMasterId }" name="<portlet:namespace/>reportMasterId" class="reportMasterId"/>
                        ...
                        <div class="nps-input-box file-upload">
                            ...
                            <div class="file-select">
                               ...
                               <input type="file" class="chooseFile" id="chooseFile" name="BenchmarkFile" />                               
                            </div>
                            <label id="error-BenchmarkFile" class="error-message text-danger"></label>
                        </div>
						...
						<div class="col-md-6">
							<div class="form-group">
								<div class="fileUploadInput">
									<input type="file" class="form-control" accept="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"name="BenchmarkFile" />
									<button type="button" class="btn file-btn">Choose File</button>
								</div>
								<label id="error-BenchmarkFile" class="error-message"></label>
							</div>
						</div>
					</div>
...
$(function(){
    $('#upload').on('click', function(){
    	...
    	if ($('input[name="BenchmarkFile"]').get(0).files.length === 0) {
    	    console.log("No files selected.");
    	    $("#error-BenchmarkFile").html("Please select a file to upload");
    	    return false;
    	}else if($('input[name="BenchmarkFile"]').get(0).files[0].name != "Benchmark return for NPS Schemes.xlsx"){
    		console.log("No files selected.");
    	    $("#error-BenchmarkFile").html("Please upload Benchmark return for NPS Schemes.xlsx");
    	    return false;
    	}
        ...
        $(".content").hide();
        $(".animaion").show();
        $("#error-BenchmarkFile").html("");
        $.ajax({
        	type: "POST",
            processData: false,
            contentType: false,
            cache: false,
            url: '${benchmarkFileURl}',
            data: fd,
