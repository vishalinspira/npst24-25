<%@page import="com.detailedauditreport.constants.DetailedAuditReportPortletKeys"%>
<%@ include file="/init.jsp" %>

<portlet:resourceURL id="<%=DetailedAuditReportPortletKeys.DetailedAuditReport%>" var="detailedAuditReportResourceURL" /> 
               ...
               <aui:form id="fileinfo" method="POST" >
                  ...
                  <div class="nps-input-box file-upload">
                     ...
                     <div class="file-select">
                        ...
                        <input type="file" id="chooseFile_1" name="detailedAuditReportPDFFile" accept=".pdf"/>
                     </div>
                     <label id="error-detailedAuditReportPDFFile" class="error-message text-danger"></label>
                  </div>
                  <div class="nps-input-box file-upload">
                     <label>Report </label>
                     <div class="file-select">
                        ...
                        <div class="file-select-button common-btn" id="fileName">Choose File</div>
                        <input type="file" id="chooseFile_2" name="detailedAuditReportDocFile" accept=".docx"/>
                     </div>
                     <label id="error-detailedAuditReportDocFile" class="error-message text-danger"></label>
                  </div>
...
$(function(){
    ...
    $('#upload').on('click', function(){
    	if ($('input[name="detailedAuditReportPDFFile"]').get(0).files.length === 0) {
    	    console.log("No files selected.");
    	    $("#error-detailedAuditReportPDFFile").html("Please select a file to upload");
    	    return false;
    	}else if($('input[name="detailedAuditReportPDFFile"]').get(0).files[0].name != "Detailed Audit Report.pdf"){
    		console.log("No files selected.");
    	    $("#error-detailedAuditReportPDFFile").html("Please upload Detailed Audit Report.pdf");
    	    return false;
    	}
    	
    	if ($('input[name="detailedAuditReportDocFile"]').get(0).files.length === 0) {
    	    console.log("No files selected.");
    	    $("#error-detailedAuditReportDocFile").html("Please select a file to upload");
    	    return false;
    	}else if($('input[name="detailedAuditReportDocFile"]').get(0).files[0].name != "Detailed Audit Report.docx"){
    		console.log("No files selected.");
    	    $("#error-detailedAuditReportDocFile").html("Please upload Detailed Audit Report.docx");
    	    return false;
    	}
        ...
        $(".content").hide();
        $(".animaion").show();
        $("#error-detailedAuditReportPDFFile").html("");
        $("#error-detailedAuditReportDocFile").html("");
        $.ajax({
            url: '${detailedAuditReportResourceURL}', 
            type: 'POST',
            data: fd,
