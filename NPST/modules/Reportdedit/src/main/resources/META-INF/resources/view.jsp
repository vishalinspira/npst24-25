...
<portlet:resourceURL id="/reportdebit/save" var="savereportdebitURL"></portlet:resourceURL>
					...
					<div class="nps-input-box mt-0">
                        ...
                        <div class="nps-input-box file-upload">
                            ...
                            <div class="file-select">
                               ...
                               <input type="file" class="chooseFile" id="chooseFile" name="reportdebitFile" />                               
                            </div>
                            <label id="error-reportdebitFile" class="error-message text-danger"></label>
                        </div>
...
$(function(){
    $('#upload').on('click', function(){
    	if ($('input[name="reportdebitFile"]').get(0).files.length === 0) {
    	    console.log("No files selected.");
    	    $("#error-reportdebitFile").html("Please select a file to upload");
    	    return false;
    	} else if($('input[name="reportdebitFile"]').get(0).files[0].name != "Valuation report Debt.xlsx"){
    		console.log("No files selected.");
    	    $("#error-reportdebitFile").html("Please upload Valuation report Debt.xlsx");
    	    return false;
    	}
        ...
        $(".content").hide();
        $(".animaion").show();
        $("#error-reportdebitFile").html("");
        $.ajax({
            url: '<%=savereportdebitURL %>',  
            type: 'POST',
            data: fd,
            success:function(data){
            	...
            	if(data.status){
            		//var pdfURL = data.pdfURL;
           		 	...
           		 	$(".formrow").show();
           		 	$('input[name="reportdebitFile"').val("");
           		 	$('#remarks').val("");
