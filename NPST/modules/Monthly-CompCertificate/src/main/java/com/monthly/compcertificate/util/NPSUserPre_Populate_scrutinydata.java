	...
	public void pre_populate_scrutiny_data(HttpServletRequest renderRequest) {
		...
		try {
			...
			if(Validator.isNotNull(reportUploadLogId) && reportUploadLogId >0) {
				...
				List<MnCompCertificateScrutiny> mnCompCertificateScrutinyList = MnCompCertificateScrutinyLocalServiceUtil.findByReportUploadLogId(reportUploadLogId);
				
				if(Validator.isNotNull(mnCompCertificateScrutinyList) && mnCompCertificateScrutinyList.size() > 0) {
					mnCompCertificateScrutiny = mnCompCertificateScrutinyList.get(mnCompCertificateScrutinyList.size()-1);
				}
				
				
				if(null != mnCompCertificateScrutiny) {
				...
				renderRequest.setAttribute("mnCompCertificateScrutiny", mnCompCertificateScrutiny);
