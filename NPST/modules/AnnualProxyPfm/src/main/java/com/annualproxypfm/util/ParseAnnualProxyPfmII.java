...
public class ParseAnnualProxyPfmII {
	...
	public static void saveSheetTwo(File file, XSSFWorkbook workbook, long userId, JSONArray errorArray, XSSFSheet xx, boolean isexcelhaserror,List<AnnualProxyPfmII> annualProxyPfmIIs, JSONArray annualProxyPfmIIJsonArray, DecimalFormat decimalFormat, Long reportUploadLogId) {
			...
			if (file != null) {
				...
				while (rows.hasNext()) {
							...
							if(rowCount > 1) {
								if (i == 0) {
									if (val != null && Validator.isNotNull(val) && val.length() > 0) {
										...
										BigDecimal  sr_no =  (BigDecimal) decimalFormat.parse(val);
									...
									BigDecimal  total_number_of_resolution =  (BigDecimal) decimalFormat.parse(val);
									annualProxyPfmII.setTotal_number_of_resolution(total_number_of_resolution);
								}
								
								else if (i == 5) {
									BigDecimal  for_ =  (BigDecimal) decimalFormat.parse(val);
									annualProxyPfmII.setFor_(for_);
								}
								else if (i == 6) {
									BigDecimal  against =  (BigDecimal) decimalFormat.parse(val);
									annualProxyPfmII.setAgainst(against);
								}
								else if (i == 7) {
									BigDecimal  abstained =  (BigDecimal) decimalFormat.parse(val);
					...
					if (isError && rowCount > 1) {
						...
						JSONObject annualProxyPfmIIJsonObject = JSONFactoryUtil.createJSONObject(annualProxyPfmII.toString());
						annualProxyPfmIIJsonArray.put(annualProxyPfmIIJsonObject);
