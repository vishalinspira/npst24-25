...
import com.mn.nonunanimous.voting.constants.MonthlyNonUnanimousVotingPortletKeys;
...
@Component(property = { 
		"javax.portlet.name=" + MonthlyNonUnanimousVotingPortletKeys.MONTHLYNONUNANIMOUSVOTING,
		"mvc.command.name=" + MonthlyNonUnanimousVotingPortletKeys.nonUnanimous, 
		}, 
service = MVCResourceCommand.class)

public class NonUnanimous implements MVCResourceCommand {
	...
	public JSONObject fileUpload(ThemeDisplay themeDisplay, ResourceRequest resourceRequest) {
		...
		List<MnNonUnanimous> mnUnanimousList = new ArrayList<MnNonUnanimous>();
										...
										if (i == 0) {
											if (val != null && Validator.isNotNull(val) && val.length() > 0) {
												...
												mnUnanimous.setS_no(iValue);
													...
													resultJsonObject.put("status", false);
													resultJsonObject.put("msg", CommonParser.bigDecimalPattern);
													return resultJsonObject;
												}
											}
//											 else {
...
//												Date date_1=null;
												try {
													...
													mnUnanimous.setMeeting_date(cell.getDateCellValue());
												} catch (Exception e) {
											...
											try {
												...
												mnUnanimous.setResolution_no(res_no);
											} catch (Exception e) {
		                                	...
		                                	mnUnanimous.setSes_id(val);
										}
		                                else if (i == 13) {
		                                	mnUnanimous.setSes_res_id(val);
										}
		                                else if (i == 14) {
												try {
													Date date_1 = cell.getDateCellValue();
													mnUnanimous.setReportingDate(date_1);
												} catch (Exception e) {
													_log.info("error parsing date--->"+val);
													resultJsonObject.put("status", false);
													resultJsonObject.put("msg", CommonParser.dateExceptionMsg);
						...
						_log.info("mnUnanimous" +" rowcount"+rowCount);
					}
				...
				if (!isexcelhaserror) {
					MnNonUnanimousLocalServiceUtil.addMnNonUnanimous(mnUnanimousList);
					_log.info("mnUnanimous" +" datacount"+mnUnanimousList.size());
	...
	public long uploadFILETOFOLDER(ThemeDisplay themeDisplay, ResourceRequest resourceRequest) {
		...
		String fileName = date.getTime() + uploadPortletRequest.getFileName("nonUnanimousFile");
		...
		String description = "nonUnanimousFile";
