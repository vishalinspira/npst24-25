<%@page import="PFM_SBIRequestForDataReport.constants.PFM_SBIRequestForDataReportPortletKeys"%>
<%@ include file="/init.jsp" %>
<!-- <portlet:resourceURL id="/annexureiva/save" var="annexureivaURL"></portlet:resourceURL> -->

<portlet:resourceURL id="<%= PFM_SBIRequestForDataReportPortletKeys.savesbidatareport%>" var="saveSBIReportURL" />
                     ...
                     <aui:form id="fileinfo" method="POST" >
                        ...
                        <div class="nps-input-box file-upload">
                           ...
                           <div class="file-select">
                              ...
                              <input type="file" id="chooseFile" name="SBIRequestForDataReport" />
                           </div>
                           <label id="error-SBIRequestForDataReport" class="error-message text-danger"></label>
                        </div>
	...
	<div class="row">
	    <div class="col">
	        <div class="page_title">
	            <img src="/o/Liferay-NPS-Theme/images/file-upload.png" alt="Upload Reports" /> SBI RequestForDataReport
	        </div>
	    </div>
	</div>
						...
						<div class="col-md-6">
							<div class="form-group">
								<div class="fileUploadInput">
									<input type="file" class="form-control" name="SBIRequestForDataReport" />
									<button type="button" class="btn file-btn">Choose File</button>
								</div>
								<label id="error-SBIRequestForDataReport" class="error-message"></label>
							</div>
						</div>
					</div>
  ...
  $(document).ready(function() {
	...
	$(function(){
	    $('#upload').on('click', function(){
	    	if ($('input[name="SBIRequestForDataReport"]').get(0).files.length === 0) {
	    	    console.log("No files selected.");
	    	    $("#error-SBIRequestForDataReport").html("Please select a file to upload");
	    	    return false;
	    	}
	    	else if($('input[name="SBIRequestForDataReport"]').get(0).files[0].name != "Monthly scheme data.xlsx"){
	    		console.log("No files selected.");
	    		$("#error-SBIRequestForDataReport").html("Please upload Monthly scheme data.xlsx");
	    	    return false;
	    	}
	        ...
	        $(".content").hide();
	        $(".animaion").show();
	        $("#error-SBIRequestForDataReport").html("");
	        $.ajax({
