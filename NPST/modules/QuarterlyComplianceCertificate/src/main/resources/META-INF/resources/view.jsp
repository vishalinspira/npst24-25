<%@page import="com.quarterlycompliancecertificate.constants.QuarterlyComplianceCertificatePortletKeys"%>
<%@ include file="/init.jsp" %>

<portlet:resourceURL id="<%=QuarterlyComplianceCertificatePortletKeys.quarterlyComplianceCertificate%>" var="quarterlyComplianceCertificateResourceURL" /> 
               ...
               <aui:form id="fileinfo" method="POST" >
                  ...
                  <div class="nps-input-box file-upload">
                     ...
                     <div class="file-select">
                        ...
                        <input type="file" id="chooseFile" name="quarterlyComplianceCertificate" accept=".pdf"/>
                     </div>
                     <label id="error-quarterlyComplianceCertificate" class="error-message text-danger"></label>
                  </div>
...
$(function(){
    ...
    $('#upload').on('click', function(){
    	if ($('input[name="quarterlyComplianceCertificate"]').get(0).files.length === 0) {
    	    console.log("No files selected.");
    	    $("#error-quarterlyComplianceCertificate").html("Please select a file to upload");
    	    return false;
    	}else if($('input[name="quarterlyComplianceCertificate"]').get(0).files[0].name != "Quarterly Compliance Certificate.pdf"){
    		console.log("No files selected.");
    	    $("#error-quarterlyComplianceCertificate").html("Please upload Quarterly Compliance Certificate.pdf");
    	    return false;
    	}
        ...
        $(".content").hide();
        $(".animaion").show();
        $("#error-quarterlyComplianceCertificate").html("");
        $.ajax({
            url: '${quarterlyComplianceCertificateResourceURL}', 
            type: 'POST',
            data: fd,
            success:function(data){
            	...
            	if(data.status){
            		$('#success_tic').modal('show');
           		 	$(".formrow").show();
           		 //	$('input[name="quarterlyComplianceCertificate"').val("");
           		// 	$('#remarks').val("");
            	}else{
            	//	console.log(data);
