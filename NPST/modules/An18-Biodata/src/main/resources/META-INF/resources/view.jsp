<%@page import="An18.Biodata.constants.An18BiodataPortletKeys"%>
<%@ include file="/init.jsp" %>

<portlet:resourceURL id="<%=An18BiodataPortletKeys.biodata%>" var="PDFResourceURL" />
<portlet:resourceURL id="<%=An18BiodataPortletKeys.getReportUpload%>" var="GetReportResourceURL" />  
<!-- Modal -->
        ...
        <div class="row">
	     <div class="col-12">
	        <div class="text-right">
	            <p  class="back_btn"><a class="backbtn" href="/web/guest/periodically-average"><i class="fas fa-arrow-left text-white pr-3"></i>Back</a></p>
	        </div>
	     </div>
	  </div>
   <div class="row">
                     ...
                     <aui:form id="fileinfo" method="POST" >
                        ...
                        <div class="nps-input-box file-upload">
                           ...
                           <div class="file-select">
                              ...
                              <input type="file" id="chooseFile" name="annexEighteenFile" />
                           </div>
                           <label id="error-annexEighteenFile" class="error-message text-danger"></label>
                        </div>
	...
	<div class="row">
	    <div class="col">
	        <div class="page_title">
	            <img src="/o/Liferay-NPS-Theme/images/file-upload.png" alt="Upload Reports" />Bio data of director
	        </div>
	    </div>
	</div>
						...
						<div class="col-md-6">
							<div class="form-group">
								<div class="fileUploadInput">
									<input type="file" class="form-control" name="annexEighteenFile" />
									<button type="button" class="btn file-btn">Choose File</button>
								</div>
								<label id="error-annexEighteenFile" class="error-message"></label>
							</div>
						</div>
					</div>
...
$(function(){
    ...
    $('#upload').on('click', function(){
    	if ($('input[name="annexEighteenFile"]').get(0).files.length === 0) {
    	    console.log("No files selected.");
    	    $("#error-annexEighteenFile").html("Please select a file to upload");
    	    return false;
    	}else if($('input[name="annexEighteenFile"]').get(0).files[0].name != "Bio data of director-converted.pdf"){
    		console.log("No files selected.");
    	    $("#error-annexEighteenFile").html("Please upload Bio data of director-converted.pdf");
    	    return false;
    	}
            ...
            success:function(data){
            	...
            	if(data.status){
            		//var pdfURL = data.pdfURL;
           		 	...
           		 	$(".formrow").show();
           		 	$('input[name="annexEighteenFile"').val("");
           		 	$('#remarks').val("");
            	}else{
            		console.log(data);
