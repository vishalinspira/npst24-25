package com.griev.pending.resource;
...
import com.griev.pending.constants.GrievPendingPortletKeys;
...
@Component(property = { "javax.portlet.name=" + GrievPendingPortletKeys.GRIEVPENDING,
"mvc.command.name=/grievpending/save" }, service = MVCResourceCommand.class)
public class SaveGrievPending implements MVCResourceCommand{
	@Override
	...
	public JSONObject fileUpload(ThemeDisplay themeDisplay, ResourceRequest resourceRequest) {
		...
		String fileName = uploadPortletRequest.getFileName("grievpendingFile");

		File file = uploadPortletRequest.getFile("grievpendingFile");
		

		String mimeType = uploadPortletRequest.getContentType("grievpendingFile");
		...
		JSONArray grievpendingOneJsonArray= JSONFactoryUtil.createJSONArray();
		List<GrievPendingOne> grievpendingones= new ArrayList<GrievPendingOne>();
		
		JSONArray grievPendingTwoJsonArray= JSONFactoryUtil.createJSONArray();
		List<GrievPendingTwo> grievPendingTwos= new ArrayList<GrievPendingTwo>();
		
		JSONArray grievPendingSummaryJsonArray= JSONFactoryUtil.createJSONArray();
		List<GrievPendingSummary> grievPendingSummarys= new ArrayList<GrievPendingSummary>();
		...
		ArrayList <String> list = new ArrayList<String>();
	      list.add("Summary");
	      list.add("NPS Nodal Office");
	      list.add("APY and NPS Lite");
		...
		try {
			if (file != null) {
				...
				_log.info("file is blank.");
			}
		}catch (Exception e) {
			_log.error(e);
		}
		//deleting old sheet data
		GrievPendingOneLocalServiceUtil.deleteGrievPendingOneByReportUploadLogId(reportUploadLogId);
		GrievPendingSummaryLocalServiceUtil.deleteGrievPendingSummaryByReportUploadLogId(reportUploadLogId);
		GrievPendingTwoLocalServiceUtil.deleteGrievPendingTwoByReportUploadLogId(reportUploadLogId);
		
		
		//parsing sheetone in util class
		
		resultJsonObject = ParseGrievPendingSheetOne.saveSheetOne(file, workbook, userId, errorArray, xx, isexcelhaserror, grievpendingones, grievpendingOneJsonArray,decimalFormat, cra, reportUploadLogId);
		if(!resultJsonObject.getBoolean("status"))
			return resultJsonObject;
		resultJsonObject = ParseGrievPendingSheetSummary.saveSheetSummary(file, workbook, userId, errorArray, xx, isexcelhaserror, grievPendingSummarys, grievPendingSummaryJsonArray, decimalFormat, cra, reportUploadLogId);
		if(!resultJsonObject.getBoolean("status"))
			return resultJsonObject;
		resultJsonObject = ParseGrievPendingSheetTwo.saveSheetTwo(file, workbook, userId, errorArray, xx, isexcelhaserror, grievPendingTwoJsonArray, grievPendingTwos,decimalFormat, cra, reportUploadLogId);
		if(!resultJsonObject.getBoolean("status"))
			return resultJsonObject;
		_log.info(grievpendingOneJsonArray.toString());
		
		if (!isexcelhaserror) {
			
			GrievPendingSummaryLocalServiceUtil.addGrievPendingSummarys(grievPendingSummarys);
			try {
				GrievPendingOneLocalServiceUtil.addGrievPendingOnes(grievpendingones);
				GrievPendingTwoLocalServiceUtil.addGrievPendingTwos(grievPendingTwos);
			} catch (Exception e1) {
				resultJsonObject.put("status", false);
	...
	public long uploadFILETOFOLDER(ThemeDisplay themeDisplay, ResourceRequest resourceRequest) {
		...
		String fileName = date.getTime()+ uploadPortletRequest.getFileName("grievpendingFile");
		...
		String description = "grievpendingFile";
