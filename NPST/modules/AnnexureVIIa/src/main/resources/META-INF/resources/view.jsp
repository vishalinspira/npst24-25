...
<portlet:resourceURL id="/annexureviia/save" var="saveannexureviiaURL"></portlet:resourceURL>
        ...
        <div class="row">
            <div class="col-lg-7 col-md-10 col-sm-12 col-12">
                <div class="nps-report">
                    <h4>D-Remit returns monthly MIS</h4>
                    <div class="content">
					...
					<input type="hidden" value="${reportMasterId }" name="<portlet:namespace/>reportMasterId" class="reportMasterId"/>
                        ...
                        <div class="nps-input-box file-upload">
                            ...
                            <div class="file-select">
                               ...
                               <input type="file" class="chooseFile" id="chooseFile" name="annexureviiaFile" />                               
                            </div>
                            <label id="error-annexureviiaFile" class="error-message text-danger"></label>
                        </div>
		...
		<div class="row">
		    <div class="col">
		        <div class="page_title">
		            <img src="/o/Liferay-NPS-Theme/images/file-upload.png" alt="Upload Reports" /> D-Remit returns monthly MIS
		        </div>
		    </div>
		</div>
								...
								<div class="col-md-6">
									<div class="form-group">
										<div class="fileUploadInput">
											<input type="file" class="form-control" accept="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" name="annexureviiaFile" />
											<button type="button" class="btn file-btn">Choose File</button>
										</div>
										<label id="error-annexureviiaFile" class="error-message"></label>
									</div>
								</div>
							</div>
...
$(function(){
    ...
    $('#upload').on('click', function(){
    	if ($('input[name="annexureviiaFile"]').get(0).files.length === 0) {
    	    console.log("No files selected.");
    	    $("#error-annexureviiaFile").html("Please select a file to upload");
    	    return false;
    	} else if($('input[name="annexureviiaFile"]').get(0).files[0].name != "D-Remit returns monthly MIS.xlsx"){
    		console.log("No files selected.");
    	    $("#error-annexureviiaFile").html("Please upload D-Remit returns monthly MIS.xlsx");
    	    return false;
    	}
            ...
            success:function(data){
            	...
            	if(data.status){
            		//var pdfURL = data.pdfURL;
           		 	...
           		 	$(".formrow").show();
           		 	$('input[name="annexureviiaFile"').val("");
           		 	$('#remarks').val("");
