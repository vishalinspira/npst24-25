<%@page import="com.benchmarkMonthly.resource.CheckRoleUtil"%>
...
<portlet:resourceURL id="/saveBenchmarkMonthly/data" var="BenchmarkFileMonthlyFileURl"></portlet:resourceURL>
        ...
        <div class="row">
            <div class="col-lg-7 col-md-10 col-sm-12 col-12">
                <div class="nps-report">
                    <div class="content">
                    <h4>Benchmark Returns for NPS Schemes</h4>
					...
					<input type="hidden" value="${reportMasterId }" name="<portlet:namespace/>reportMasterId" class="reportMasterId"/>
                        ...
                        <div class="nps-input-box file-upload">
                            ...
                            <div class="file-select">
                               ...
                               <input type="file" class="chooseFile" id="chooseFile" name="BenchmarkFileMonthly" />                               
                            </div>
                            <label id="error-BenchmarkFileMonthly" class="error-message text-danger"></label>
                        </div>
	...
	<div class="row">
	    <div class="col">
	        <div class="page_title">
	            <img src="/o/Liferay-NPS-Theme/images/file-upload.png" alt="Upload Reports" /> Benchmark index returns for the NPS schemes
	        </div>
	    </div>
	</div>
						...
						<div class="col-md-6">
							<div class="form-group">
								<div class="fileUploadInput">
									<input type="file" class="form-control" accept="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"name="BenchmarkFileMonthly" />
									<button type="button" class="btn file-btn">Choose File</button>
								</div>
								<label id="error-BenchmarkFileMonthly" class="error-message"></label>
							</div>
						</div>
					</div>
...
$(function(){
    $('#upload').on('click', function(){
    	...
    	if ($('input[name="BenchmarkFileMonthly"]').get(0).files.length === 0) {
    	    console.log("No files selected.");
    	    $("#error-BenchmarkFileMonthly").html("Please select a file to upload");
    	    return false;
    	}else if($('input[name="BenchmarkFileMonthly"]').get(0).files[0].name != "Benchmark index returns for the NPS schemes.xlsx"){ 
    		console.log("No files selected.");
    	    $("#error-BenchmarkFileMonthly").html("Please upload Benchmark returns for the NPS schemes.xlsx");
    	    return false;
    	}
        ...
        $(".content").hide();
        $(".animaion").show();
        $("#error-BenchmarkFileMonthly").html("");
        $.ajax({
        	type: "POST",
            processData: false,
            contentType: false,
            cache: false,
            url: '${BenchmarkFileMonthlyFileURl}',
            data: fd,
