...
<%
	...
	boolean isPfmSupervisor = RoleServiceUtil.hasUserRole(user.getUserId(), user.getCompanyId(), NPSTRoleConstants.GRIEVANCES_CHECKER, false);
...
for(int i=0; i<urlArray.length();i++){
	...
	try{
		 dlFileEntry= DLFileEntryLocalServiceUtil.getDLFileEntry(Long.valueOf(fileEntryId));
		
	}catch(Exception e){}
%>

<%if(Validator.isNotNull(dlFileEntry)){ %>
	<span class="file-preview-span" data-index='<%= i%>'><a href="<%=object.getString("downloadURL")%>"> <%=RegexUtil.removeStartingDigits(dlFileEntry.getTitle())%></a></span>
	<%} %>
