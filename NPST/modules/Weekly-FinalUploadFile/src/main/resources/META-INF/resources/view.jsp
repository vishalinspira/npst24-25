<%@page import="Weekly.FinalUploadFile.constants.WeeklyFinalUploadFilePortletKeys"%>
<%@ include file="/init.jsp" %>

<portlet:resourceURL id="<%=WeeklyFinalUploadFilePortletKeys.finalUpload%>" var="finalUploadResourceURL" /> 
<!-- 1. -->
<!-- Modal -->
	...
	<div class="row">
	    <div class="col">
	        <div class="page_title">
	            <img src="/o/Liferay-NPS-Theme/images/file-upload.png" alt="Upload Reports" /> Weekly - Final Upload File
	        </div>
	    </div>
	</div>
						...
						<div class="col-md-6">
							<div class="form-group">
								<div class="fileUploadInput">
									<input type="file" class="form-control" name="finalUploadFile" />
									<button type="button" class="btn file-btn">Choose File</button>
								</div>
								<label id="error-finalUploadFile" class="error-message"></label>
							</div>
						</div>
					</div>
  ...
  $(document).ready(function() {
	...
	$(function(){
	    $('#upload').on('click', function(){
	    	if ($('input[name="finalUploadFile"]').get(0).files.length === 0) {
	    	    console.log("No files selected.");
	    	    $("#error-finalUploadFile").html("Please select a file to upload");
	    	    return false;
	    	}
	    	else if($('input[name="finalUploadFile"]').get(0).files[0].name != "19Nov-2021_Final upload file.xlsx"){
	    		console.log("No files selected.");
	    		$("#error-finalUploadFile").html("Please upload 19Nov-2021_Final upload file.xlsx");
	    	    return false;
	    	}
	        ...
	        $.ajax({
	            ...
	            success:function(data){
	            	...
	            	if(data.status){
	           		 	...
	           		 	$(".formrow").show();
	           		 	$('input[name="finalUploadFile"').val("");
		           		setTimeout(function() {
