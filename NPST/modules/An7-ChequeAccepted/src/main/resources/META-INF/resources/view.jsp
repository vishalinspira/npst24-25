<%@page import="An7.ChequeAccepted.constants.An7ChequeAcceptedPortletKeys"%>
<%@ include file="/init.jsp" %>

<!-- <portlet:resourceURL id="/annexureiva/save" var="annexureivaURL"></portlet:resourceURL> -->
<portlet:resourceURL id="<%=An7ChequeAcceptedPortletKeys.chequeAccepted%>" var="chequeAccepted" /> 
<!-- 1. -->
<!-- Modal -->
	...
	<div class="row">
	    <div class="col">
	        <div class="page_title">
	            <img src="/o/Liferay-NPS-Theme/images/file-upload.png" alt="Upload Reports" /> Annexure7 - All Cheque Accepted
	        </div>
	    </div>
	</div>
						...
						<div class="col-md-6">
							<div class="form-group">
								<div class="fileUploadInput">
									<input type="file" class="form-control" name="chequeAcceptedFile" />
									<button type="button" class="btn file-btn">Choose File</button>
								</div>
								<label id="error-chequeAcceptedFile" class="error-message"></label>
							</div>
						</div>
					</div>
  ...
  $(document).ready(function() {
	...
	$(function(){
	    $('#upload').on('click', function(){
	    	if ($('input[name="chequeAcceptedFile"]').get(0).files.length === 0) {
	    	    console.log("No files selected.");
	    	    $("#error-chequeAcceptedFile").html("Please select a file to upload");
	    	    return false;
	    	}
	    	else if($('input[name="chequeAcceptedFile"]').get(0).files[0].name != "Annexure 7 MIS Collection Rejection (monthly).xlsx"){
	    		console.log("No files selected.");
	    		$("#error-chequeAcceptedFile").html("Please upload Annexure 7 MIS Collection Rejection (monthly).xlsx");
	    	    return false;
	    	}
	        ...
	        $.ajax({
	            ...
	            success:function(data){
	            	...
	            	if(data.status){
	           		 	...
	           		 	$(".formrow").show();
	           		 	$('input[name="chequeAcceptedFile"').val("");
	           		 	setTimeout(function() {
