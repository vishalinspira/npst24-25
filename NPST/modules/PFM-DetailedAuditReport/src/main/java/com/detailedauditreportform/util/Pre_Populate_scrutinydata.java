...
import com.daily.average.service.service.DARScrutinyLocalServiceUtil;
	...
	public void pre_populate_scrutiny_data(HttpServletRequest renderRequest) {
		...
		try {
			...
			if(Validator.isNotNull(reportUploadLogs) && reportUploadLogs.size() >0) {
				...
				List<DARScrutiny> darScrutinyList = DARScrutinyLocalServiceUtil.findByReportUploadLogId(reportUploadLogId);
				
				if(Validator.isNotNull(darScrutinyList) && darScrutinyList.size() > 0) {
					darScrutiny = darScrutinyList.get(darScrutinyList.size()-1);
				}
				...
				renderRequest.setAttribute("darScrutiny", darScrutiny);
