package com.detailedauditreportform.handler;
...
public class DARHandler extends BaseWorkflowHandler<DAR> {
	
	DARLocalService darLocalService;
	
	@Reference(unbind = "-")
	public void setDARLocalService(DARLocalService darLocalService) {
		this.darLocalService = darLocalService;
	}
	...
	@Override
	public DAR updateStatus(int status, Map<String, Serializable> workflowContext) throws PortalException {
		...
		DAR dar = darLocalService.updateDARStatus(id, userId, status, serviceContext, WorkflowContextUtil.convert(workflowContext));

		return dar;
	}
	
	Log _log = LogFactoryUtil.getLog(DARHandler.class);
