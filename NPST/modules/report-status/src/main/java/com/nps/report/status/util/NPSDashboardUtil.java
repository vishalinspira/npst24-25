...
public class NPSDashboardUtil {
	...
	public String getCompanyName(long companyId, long userId) {
		try {
			boolean npst = userLocalService.hasRoleUser(companyId, NPSTRoleConstants.NPST, userId, Boolean.FALSE);
			if(npst)
		...
		try {
			boolean pfm = userLocalService.hasRoleUser(companyId, NPSTRoleConstants.PFM, userId, Boolean.FALSE);
			if(pfm)
		...
		try {
			boolean cra = userLocalService.hasRoleUser(companyId, NPSTRoleConstants.CRA, userId, Boolean.FALSE);
			if(cra)
		...
		try {
			boolean custodian = userLocalService.hasRoleUser(companyId, NPSTRoleConstants.CUSTODIAN, userId, Boolean.FALSE);
			if(custodian)
		...
		try {
			boolean grievances = userLocalService.hasRoleUser(companyId, NPSTRoleConstants.GRIEVANCES, userId, Boolean.FALSE);
			if(grievances)
	...
	public String getUserName(long classPK, String className) {
		...
		if(className.equalsIgnoreCase(Role.class.getName())) {
			...
			//long[] userIds = userLocalService.getRoleUserIds(classPK);
	...
	public boolean isNonNpsUser(long userId) {
		...
		try {
			...
			if(Validator.isNotNull(companyId) && !isNonNpsUser) {
				...
				boolean pfmChecker = userLocalService.hasRoleUser(companyId, NPSTRoleConstants.CHECKER, userId, inherited);
				boolean pfmSupervisor = userLocalService.hasRoleUser(companyId, NPSTRoleConstants.SUPERVISOR, userId, inherited);
				isNonNpsUser = (pfmChecker || pfmSupervisor || pfmMaker);
			}
			if(Validator.isNotNull(companyId) && !isNonNpsUser) {
				boolean pfmMaker = userLocalService.hasRoleUser(companyId, NPSTRoleConstants.GRIEVANCES_MAKER, userId, inherited);
				boolean pfmChecker = userLocalService.hasRoleUser(companyId, NPSTRoleConstants.GRIEVANCES_CHECKER, userId, inherited);
				boolean pfmSupervisor = userLocalService.hasRoleUser(companyId, NPSTRoleConstants.GRIEVANCES_SUPERVISOR, userId, inherited);
				isNonNpsUser = (pfmChecker || pfmSupervisor || pfmMaker);
			}
		} catch (Exception e) {
