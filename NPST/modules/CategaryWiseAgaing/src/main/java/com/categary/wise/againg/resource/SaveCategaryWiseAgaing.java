package com.categary.wise.againg.resource;
...
import com.daily.average.service.service.CategarywiseTwoLocalServiceUtil;
...
@Component(property = { "javax.portlet.name=" + categarywiseagaingPortletKeys.CATEGARYWISEAGAING,
"mvc.command.name=/categarywie/save" }, service = MVCResourceCommand.class)

public class SaveCategaryWiseAgaing implements MVCResourceCommand{
	@Override
	...
	public JSONObject fileUpload(ThemeDisplay themeDisplay, ResourceRequest resourceRequest) {
		...
		String fileName = uploadPortletRequest.getFileName("categarywieFile");

		File file = uploadPortletRequest.getFile("categarywieFile");
		//excelValidationAn7Api.validateExcelFile(file);

		String mimeType = uploadPortletRequest.getContentType("categarywieFile");
		...
		if(resultJsonObject.getBoolean("status")) {
			...
			JSONArray categarywiseOneJsonArray= JSONFactoryUtil.createJSONArray();
			List<CategarywiseOne> categarywiseOnes= new ArrayList<CategarywiseOne>();
			
			JSONArray categarywiseTwoJsonArray= JSONFactoryUtil.createJSONArray();
			List<CategarywiseTwo> categarywiseTwos= new ArrayList<CategarywiseTwo>();
			...
			ArrayList <String> list = new ArrayList<String>();
		      list.add("Raised against NPS Trust");
		      list.add("Escalated to NPS Trust");
			...
			CategarywiseOneLocalServiceUtil.deleteCategarywiseOneByReportUploadLogId(reportUploadLogId);
			CategarywiseTwoLocalServiceUtil.deleteCategarywiseTwoByReportUploadLogId(reportUploadLogId);
			
			//parsing sheet one in util class		
			resultJsonObject = ParseCategaryWiseAgaingOne.saveSheetOne(file, workbook, userId, errorArray, xx, isexcelhaserror, categarywiseOnes, categarywiseOneJsonArray, decimalFormat, reportUploadLogId, cra);
			if(!resultJsonObject.getBoolean("status"))
				return resultJsonObject;
			
			resultJsonObject = ParseCategaryWiseAgaingTwo.saveSheetTwo(file, workbook, userId, errorArray, xx, isexcelhaserror, categarywiseTwos, categarywiseTwoJsonArray, decimalFormat, reportUploadLogId, cra);
			if(!resultJsonObject.getBoolean("status"))
				return resultJsonObject;
			
			
			_log.info(categarywiseOneJsonArray.toString());
			
			if (!isexcelhaserror) {
				
				
				try {
					CategarywiseOneLocalServiceUtil.addCategarywiseOnes(categarywiseOnes);
					CategarywiseTwoLocalServiceUtil.addCategarywiseTwos(categarywiseTwos);
				} catch (Exception e1) {
					resultJsonObject.put("status", false);
	...
	public long uploadFILETOFOLDER(ThemeDisplay themeDisplay, ResourceRequest resourceRequest) {
		...
		String fileName = date.getTime()+ uploadPortletRequest.getFileName("categarywieFile");
		...
		String description = "categarywieFile";
