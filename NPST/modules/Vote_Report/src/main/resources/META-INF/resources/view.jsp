...
<portlet:resourceURL id="/saveVoteReportfile/data" var="SaveVoteReportfileURL"></portlet:resourceURL>
						...
						<input type="hidden" value="${reportMasterId }" name="<portlet:namespace/>reportMasterId" class="reportMasterId"/>
	                      ...
	                      <div class="nps-input-box file-upload">
	                          ...
	                          <div class="file-select">
	                             ...
	                             <input type="file" id="chooseFile" name="votereportfile" />
	                          </div>
	                          <label id="error-votereportfile" class="error-message text-danger"></label>
	                      </div>
...
$(function(){
    ...
    $('#upload').on('click', function(){ 
    	...
    	if ($('input[name="votereportfile"]').get(0).files.length === 0) {
    	    console.log("No files selected.");
    	    $("#error-votereportfile").html("Please select a file to upload");
    	    return false;
    	}else if($('input[name="votereportfile"]').get(0).files[0].name != "Monthly Combined Vote Report.xlsx"){
    		console.log("No files selected.");
    	    $("#error-votereportfile").html("Please upload Monthly Combined Vote Report.xlsx");
    	    return false;
    	}
        var fd = new FormData($("form.form")[0]);
        $(".content").hide();
        $(".animaion").show();
        $("#error-votereportfile").html("");
        $.ajax({
        	type: "POST",
            processData: false,
            contentType: false,
            cache: false,
            url: '${SaveVoteReportfileURL}',
            data: fd,
