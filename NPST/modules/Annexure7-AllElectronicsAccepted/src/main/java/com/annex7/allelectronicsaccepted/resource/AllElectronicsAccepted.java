package com.annex7.allelectronicsaccepted.resource;
...
import Annexure7.AllElectronicsAccepted.constants.Annexure7AllElectronicsAcceptedPortletKeys;
import be.quodlibet.boxable.BaseTable;
import be.quodlibet.boxable.Cell;
import be.quodlibet.boxable.HorizontalAlignment;
import be.quodlibet.boxable.line.LineStyle;
import compliance.service.model.ElectronicAccepted;
import compliance.service.service.ElectronicAcceptedLocalService;

@Component(property = { 
		"javax.portlet.name=" + Annexure7AllElectronicsAcceptedPortletKeys.ANNEXURE7ALLELECTRONICSACCEPTED,
		"mvc.command.name=" + Annexure7AllElectronicsAcceptedPortletKeys.electronicsAccepted, 
		}, 
service = MVCResourceCommand.class)

public class AllElectronicsAccepted implements MVCResourceCommand {
	
	private static Log _log = LogFactoryUtil.getLog(AllElectronicsAccepted.class);
	...
	public JSONObject fileUpload(ThemeDisplay themeDisplay, ResourceRequest resourceRequest) {
		...
		String fileName = uploadPortletRequest.getFileName("electronicsAcceptedFile");

		File file = uploadPortletRequest.getFile("electronicsAcceptedFile");

		String mimeType = uploadPortletRequest.getContentType("electronicsAcceptedFile");

		String title = fileName;

		JSONArray eleAcceptedJsonArray = JSONFactoryUtil.createJSONArray();
		List<ElectronicAccepted> allElectronics = new ArrayList<ElectronicAccepted>();
		...
		try {
			if (file != null) {
				...
				while (rows.hasNext()) {
					...
					ElectronicAccepted electronicsAccepted = (ElectronicAccepted) electronicAcceptedLocalService.createElectronicAccepted(CounterLocalServiceUtil.increment(ElectronicAccepted.class.getName()));
					...
					electronicsAccepted.setCreatedon(new Date());
						...
						if (isError && rowCount > 3) {
							...
							eleAcceptedJsonArray.put(eleAcceptedJsonObject);
							allElectronics.add(electronicsAccepted);
						}
						rowCount++;
					}
			...
			_log.info(eleAcceptedJsonArray.toString());
			
			if (!isexcelhaserror) {
				electronicAcceptedLocalService.addElectronicsAccepted(allElectronics);
...
//	public String pdfTable(List<ElectronicAccepted> eleAccepted, ThemeDisplay themeDisplay, ResourceRequest resourceRequest) throws IOException {
...
//        Cell<PDPage> cell = headerRow.createCell(100, "Annexure 7 - All Electronic Accepted");
...
//        	ElectronicAccepted elecAccept = (ElectronicAccepted) iterator.next();
//			
...
//		File eleAcceptedtempfile = File.createTempFile("All Electronic Accepted", "pdf"); /////////////
//		
//		outputStream = new FileOutputStream(eleAcceptedtempfile); ////////////
//		//document.save("C:\\Users\\deepak\\Documents\\pdf\\All Electronic Accepted.pdf");
...
//	}
	...
	public void uploadFILETOFOLDER(ThemeDisplay themeDisplay, ResourceRequest resourceRequest) {
		...
		String fileName = uploadPortletRequest.getFileName("electronicsAcceptedFile") + "_" + getRandomNumber();
		...
		String description = "Trustee Bank Annexure 7 - All Electronics Accepted";
	...
	@Reference
	ElectronicAcceptedLocalService electronicAcceptedLocalService;
