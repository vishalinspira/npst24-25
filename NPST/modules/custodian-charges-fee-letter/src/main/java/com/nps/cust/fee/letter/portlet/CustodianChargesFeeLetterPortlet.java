...
import com.nps.cust.fee.letter.constants.CustodianChargesFeeLetterPortletKeys;
...
@Component(
	...
	property = {
		...
		"javax.portlet.display-name=CustodianChargesFeeLetter",
		"javax.portlet.init-param.template-path=/",
		"javax.portlet.init-param.view-template=/view.jsp",
		"javax.portlet.name=" + CustodianChargesFeeLetterPortletKeys.CUSTODIANCHARGESFEELETTER,
		"javax.portlet.resource-bundle=content.Language",
		"javax.portlet.security-role-ref=power-user,user"
	},
	service = Portlet.class
)
public class CustodianChargesFeeLetterPortlet extends MVCPortlet {
	private static final Log LOG = LogFactoryUtil.getLog(CustodianChargesFeeLetterPortlet.class);
	...
	//	Aspose.convertPDFTODOC(fileInputStream);
		...
		try {
			...
			if(Validator.isNotNull(file)) {
				...
				jsonObject1.put("schemeName",ColumnName.SCHEME_NAME );
				jsonObject1.put("pfm_bill_amt",ColumnName.PFM_BILL_AMT );
				jsonObject1.put("cust_bill_amt", ColumnName.CUST_BILL_AMT);
				jsonObject1.put("approval_amt",ColumnName.APPROVAL_AMT );
							...
							if (cell != null) {
								...
								if(i==0) {
									isSamePFM=val.contains(pfmName)?true:false;
										...
										jsonObject.put("pfm_bill_amt",formatNumberInIndianStyle(val) );
										
										//formatNumberWithComma(val);
									}
									else if (i == 3 && isSamePFM) {
										jsonObject.put("cust_bill_amt",formatNumberInIndianStyle(val) );	
									} 
									else if (i == 4 && isSamePFM) {
										jsonObject.put("approval_amt", formatNumberInIndianStyle(val));
...
//									}
							...
							if(isSamePFM) {	
								LOG.info("schemeName: "+jsonObject.getString("schemeName")+" approval_amt : "+jsonObject.getString("approval_amt")+"  cust_bill_amt  : "+jsonObject.getString("cust_bill_amt")+"  pfm_bill_amt  :"+jsonObject.getString("pfm_bill_amt"));
			...
			File fileLetterFile=FeeLetterPDF.generateFeeLetter(pfmName,jsonArray,letterDate,receivedDate,quarter,signeturies);
