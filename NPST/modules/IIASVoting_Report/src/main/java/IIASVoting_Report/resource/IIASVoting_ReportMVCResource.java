package IIASVoting_Report.resource;
...
import IIASVoting_Report.constants.IIASVoting_ReportPortletKeys;
...
@Component(property = { 
		"javax.portlet.name=" + IIASVoting_ReportPortletKeys.IIASVOTING_REPORT,
		"mvc.command.name=" + IIASVoting_ReportPortletKeys.saveiiasVotingreport, 
		}, 
service = MVCResourceCommand.class)

public class IIASVoting_ReportMVCResource implements MVCResourceCommand{
	
	private static Log _log = LogFactoryUtil.getLog(IIASVoting_ReportMVCResource.class);
				...
				if (file != null) {
					...
					if (errorlist.size() > 0) {
						...
						while (rows.hasNext()) {
							...
							Votingreport votingdata=VotingreportLocalServiceUtil.createVotingreport(CounterLocalServiceUtil.increment(Votingreport.class.getName()));
...
//													isError = true;
											...
											try {
												...
												votingdata.setResolution_no(res_no);
											} catch (Exception e) {
		                                	...
		                                	votingdata.setSes_res_id(val);
										}
		                                else if (i == 11) {
											if (val != null && Validator.isNotNull(val) && val.length() > 0) {
												
												votingdata.setReportingDate(cell.getDateCellValue());
											}
											 else {
												 resultJsonObject.put("status", false);
													resultJsonObject.put("msg", CommonParser.dateExceptionMsg+" "+sheetname);
...
//													errorObj.put("msg", "Error in reporting date.");
//													isError = true;
												}
										}
							...
							votingdata.setCreatedate(new Date());
								...
								//	JSONObject votingJsonObject = JSONFactoryUtil.createJSONObject(votingdata.toString());
								//	votingJsonArray.put(votingJsonObject);
									votingList.add(votingdata);
								}
								_log.debug("rowCount" + rowCount);
								rowCount++;
							}
						_log.info("voting" +" rowcount"+rowCount);
					}
				...
				//_log.info(votingJsonArray.toString());
				
				if (!isexcelhaserror) {
					VotingreportLocalServiceUtil.addVotingreport(votingList);
					_log.info("voting" +" datacount"+votingList.size());
	...
	public long uploadFILETOFOLDER(ThemeDisplay themeDisplay, ResourceRequest resourceRequest) {
		...
		String fileName = date.getTime() + uploadPortletRequest.getFileName("votingreport");
		...
		String description = "votingreport";
