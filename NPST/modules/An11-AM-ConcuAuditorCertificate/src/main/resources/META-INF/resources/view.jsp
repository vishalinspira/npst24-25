<%@page import="An11.AM.ConcuAuditorCertificate.constants.An11AMConcuAuditorCertificatePortletKeys"%>
<%@ include file="/init.jsp" %>

<portlet:resourceURL id="<%=An11AMConcuAuditorCertificatePortletKeys.amConcuAuditorCertificate%>" var="PDFURL" />
<portlet:resourceURL id="<%=An11AMConcuAuditorCertificatePortletKeys.getReportUpload%>" var="GetReportURL" />  
        ...
        <div class="row">
	     <div class="col-12">
	        <div class="text-right">
	            <p  class="back_btn"><a class="backbtn" href="/web/guest/quaterly-am-report"><i class="fas fa-arrow-left text-white pr-3"></i>Back</a></p>
	        </div>
	     </div>
	  </div>
      <div class="row">
          <div class="col-lg-7 col-md-10 col-sm-12 col-12">
              <div class="nps-report">
                  <h4>Annexure 11 Concurrent Auditor's certificate</h4>
                  <div class="content">
						...
						<input type="hidden" value="${reportMasterId }" name="<portlet:namespace/>reportMasterId" class="reportMasterId"/>
	                      ...
	                      <div class="nps-input-box file-upload">
	                          ...
	                          <div class="file-select">
	                             ...
	                             <input type="file" id="chooseFile" name="annexureelevenacFile" />
	                          </div>
	                          <label id="error-annexureelevenacFile" class="error-message text-danger"></label>
	                      </div>
	...
	<div class="row">
	    <div class="col">
	        <div class="page_title">
	            <img src="/o/Liferay-NPS-Theme/images/file-upload.png" alt="Upload Reports" />Annexure 11 Concurrent Auditor's certificate
	        </div>
	    </div>
	</div>
						...
						<div class="col-md-6">
							<div class="form-group">
								<div class="fileUploadInput">
									<input type="file" class="form-control" name="annexureelevenacFile" />
									<button type="button" class="btn file-btn">Choose File</button>
								</div>
								<label id="error-annexureelevenacFile" class="error-message"></label>
							</div>
						</div>
					</div>
...
$(function(){
    ...
    $('#upload').on('click', function(){
    	if ($('input[name="annexureelevenacFile"]').get(0).files.length === 0) {
    	    console.log("No files selected.");
    	    $("#error-annexureelevenacFile").html("Please select a file to upload");
    	    return false;
    	}else if($('input[name="annexureelevenacFile"]').get(0).files[0].name != "Annexure 11 Concurrent Auditor Certificate.pdf"){
    		console.log("No files selected.");
    	    $("#error-annexureelevenacFile").html("Please upload Annexure 11 Concurrent Auditor Certificate.pdf");
    	    return false;
    	}
        ...
        $(".content").hide();
        $(".animaion").show();
        $("#error-annexureelevenacFile").html("");
        $.ajax({
            url: '${PDFURL}', 
            type: 'POST',
            data: fd,
            success:function(data){
            	...
            	if(data.status){
            		//var pdfURL = data.pdfURL;
           		 	...
           		 	$(".formrow").show();
           		 	$('input[name="annexureelevenacFile"').val("");
           		 	$('#remarks').val("");
            	}else{
            		console.log(data);
