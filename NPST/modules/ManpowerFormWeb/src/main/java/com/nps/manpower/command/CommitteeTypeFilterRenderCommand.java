...
@Component(immediate = true, property = {
		...
		"mvc.command.name=/employee/committee/filter",
}, service = MVCRenderCommand.class)
public class CommitteeTypeFilterRenderCommand implements MVCRenderCommand {

	@Override
	public String render(RenderRequest renderRequest, RenderResponse renderResponse) throws PortletException {
		logger.info("********* render command called");
		int status=ParamUtil.getInteger(renderRequest, ManpowerEmployeeFieldName.STATUS);
		...
		renderRequest.setAttribute(ManpowerEmployeeFieldName.STATUS, status);
	
		renderRequest.setAttribute(ManpowerEmployeeFieldName.PFM_NAME, pfmName);

			return jspName;
	}
	Log logger = LogFactoryUtil.getLog(CommitteeTypeFilterRenderCommand.class);
