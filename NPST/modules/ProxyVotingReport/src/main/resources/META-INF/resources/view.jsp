<%@page import="com.proxyvotingreport.constants.ProxyVotingReportPortletKeys"%>
<%@ include file="/init.jsp" %>

<portlet:resourceURL id="<%=ProxyVotingReportPortletKeys.proxyVotingDocumentUpload%>" var="proxyVotingResourceURL" /> 
               ...
               <aui:form id="fileinfo" method="POST" >
                  ...
                  <div class="nps-input-box file-upload">
                     ...
                     <div class="file-select">
                        ...
                        <input type="file" id="chooseFile_1" name="proxyVotePDFFile" accept=".pdf"/>
                     </div>
                     <label id="error-proxyVotePDFFile" class="error-message text-danger"></label>
                  </div>
                  <div class="nps-input-box file-upload">
                     <label>Report </label>
                     <div class="file-select">
                        ...
                        <input type="file" id="chooseFile_2" name="proxyVoteExcelFile" accept=".xlsx"/>
                     </div>
                     <label id="error-proxyVoteExcelFile" class="error-message text-danger"></label>
                  </div>
...
$(function(){
    ...
    $('#upload').on('click', function(){
    	if ($('input[name="proxyVotePDFFile"]').get(0).files.length === 0) {
    	    console.log("No files selected.");
    	    $("#error-proxyVotePDFFile").html("Please select a file to upload");
    	    return false;
    	}else if($('input[name="proxyVotePDFFile"]').get(0).files[0].name != "Proxy Voting Report.pdf"){
    		console.log("No files selected.");
    	    $("#error-proxyVotePDFFile").html("Please upload Proxy Voting Report.pdf");
    	    return false;
    	}
    	
    	if ($('input[name="proxyVoteExcelFile"]').get(0).files.length === 0) {
    	    console.log("No files selected.");
    	    $("#error-proxyVoteExcelFile").html("Please select a file to upload");
    	    return false;
    	}else if($('input[name="proxyVoteExcelFile"]').get(0).files[0].name != "Proxy Voting Report.xlsx"){
    		console.log("No files selected.");
    	    $("#error-proxyVoteExcelFile").html("Please upload Proxy Voting Report.xlsx");
    	    return false;
    	}
        ...
        $(".content").hide();
        $(".animaion").show();
        $("#error-proxyVotePDFFile").html("");
        $("#error-proxyVoteExcelFile").html("");
        $.ajax({
            url: '${proxyVotingResourceURL}', 
            type: 'POST',
            data: fd,
