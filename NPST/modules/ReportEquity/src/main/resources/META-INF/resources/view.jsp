...
<portlet:resourceURL id="/reportequity/save" var="savereportequityURL"></portlet:resourceURL>
	                ...
	                <div class="nps-input-box mt-0">
                       ...
                       <div class="nps-input-box file-upload">
                           ...
                           <div class="file-select">
                              ...
                              <input type="file" class="chooseFile" id="chooseFile" name="reportequityFile" />                               
                           </div>
                           <label id="error-reportequityFile" class="error-message text-danger"></label>
                       </div>
...
$(function(){
    $('#upload').on('click', function(){
    	if ($('input[name="reportequityFile"]').get(0).files.length === 0) {
    	    console.log("No files selected.");
    	    $("#error-reportequityFile").html("Please select a file to upload");
    	    return false;
    	} else if($('input[name="reportequityFile"]').get(0).files[0].name != "Valuation report Equity.xlsx"){ 
    		console.log("No files selected.");
    	    $("#error-reportequityFile").html("Please upload Valuation report Equity.xlsx");
    	    return false;
    	}
        ...
        $(".content").hide();
        $(".animaion").show();
        $("#error-reportequityFile").html("");
        $.ajax({
            url: '<%=savereportequityURL %>',  
            type: 'POST',
            data: fd,
            success:function(data){
            	...
            	if(data.status){
            		//var pdfURL = data.pdfURL;
           		 	...
           		 	$(".formrow").show();
           		 	$('input[name="reportequityFile"').val("");
           		 	$('#remarks').val("");
