		...
		<portlet:resourceURL id="/craenpsdata/save" var="savecraenpsdataURL"></portlet:resourceURL>
	...
	<div class="nps-page-main nps-upload-report">
						...
						<input type="hidden" value="${reportMasterId }" name="<portlet:namespace/>reportMasterId" class="reportMasterId"/>
	                        ...
	                        <div class="nps-input-box file-upload">
	                            ...
	                            <div class="file-select">
	                               ...
	                               <input type="file" class="chooseFile" id="chooseFile" name="craenpsdataFile" />                               
	                            </div>
	                            <label id="error-craenpsdataFile" class="error-message text-danger"></label>
	                        </div>
							...
							<div class="col-md-6">
								<div class="form-group">
									<div class="fileUploadInput">
										<input type="file" class="form-control" name="craenpsdataFile" />
										<button type="button" class="btn file-btn">Choose File</button>
									</div>
									<label id="error-craenpsdataFile" class="error-message"></label>
								</div>
							</div>
						</div>
		...
		$(function(){
		    $('#upload').on('click', function(){
		    	if ($('input[name="craenpsdataFile"]').get(0).files.length === 0) {
		    	    console.log("No files selected.");
		    	    $("#error-craenpsdataFile").html("Please select a file to upload");
		    	    return false;
		    	}else if($('input[name="craenpsdataFile"]').get(0).files[0].name != "CRA- ENPS Data input.xlsx"){
		    		console.log("No files selected.");
		    	    $("#error-craenpsdataFile").html("Please upload CRA- ENPS Data input.xlsx");
		    	    return false;
		    	}
		        ...
		        $(".content").hide();
		        $(".animaion").show();
		        $("#error-craenpsdataFile").html("");
		        $.ajax({
		            url: '<%=savecraenpsdataURL %>',  
		            type: 'POST',
		            data: fd,
		            success:function(data){
		            	...
		            	if(data.status){
		            		//var pdfURL = data.pdfURL;
		           		 	...
		           		 	$(".formrow").show();
		           		 	$('input[name="craenpsdataFile"').val("");
		            	}else{
		            		console.log(data);
