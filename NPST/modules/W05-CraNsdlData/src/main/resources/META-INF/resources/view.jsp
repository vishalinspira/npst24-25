<%@page import="W05.CraNsdlData.constants.W05CraNsdlDataPortletKeys"%>
<%@ include file="/init.jsp" %>

<!-- <portlet:resourceURL id="/annexureiva/save" var="annexureivaURL"></portlet:resourceURL> -->
<portlet:resourceURL id="<%=W05CraNsdlDataPortletKeys.craNavData%>" var="craResourceURL" /> 
<!-- 1. -->
<!-- Modal -->
                     ...
                     <aui:form id="fileinfo" method="POST" >
                        ...
                        <div class="nps-input-box file-upload">
                           ...
                           <div class="file-select">
                              ...
                              <input type="file" id="chooseFile" name="craDataFile" />
                           </div>
                           <label id="error-craDataFile" class="error-message text-danger"></label>
                        </div>
	...
	<div class="row">
	    <div class="col">
	        <div class="page_title">
	            <img src="/o/Liferay-NPS-Theme/images/file-upload.png" alt="Upload Reports" /> W05_CRA NSDL NAV data
	        </div>
	    </div>
	</div>
						...
						<div class="col-md-6">
							<div class="form-group">
								<div class="fileUploadInput">
									<input type="file" class="form-control" name="craDataFile" />
									<button type="button" class="btn file-btn">Choose File</button>
								</div>
								<label id="error-craDataFile" class="error-message"></label>
							</div>
						</div>
					</div>
  ...
  $(document).ready(function() {
	...
	$(function(){
	    $('#upload').on('click', function(){
	    	if ($('input[name="craDataFile"]').get(0).files.length === 0) {
	    	    console.log("No files selected.");
	    	    $("#error-craDataFile").html("Please select a file to upload");
	    	    return false;
	    	}
	    	else if($('input[name="craDataFile"]').get(0).files[0].name != "CRA NAV data.xlsx"){
	    		console.log("No files selected.");
	    		$("#error-craDataFile").html("Please upload CRA NAV data.xlsx");
	    	    return false;
	    	}
	        ...
	        $.ajax({
	            ...
	            success:function(data){
	            	...
	            	if(data.status){
	           		 	...
	           		 	$(".formrow").show();
	           		 	$('input[name="craDataFile"').val("");
	           			$('#remarks').val("");
	            	}else if(data.sheeterror){
	            		$('#output').html("An error occurred.Following sheets not found  "+data.errorSheetNameList);
	            		$('#errorExcel').modal('show');
	            	}else if(data.apierror){
	            		$('#output').html("An error occurred while validating the file.");
	            		$('#errorExcel').modal('show');
