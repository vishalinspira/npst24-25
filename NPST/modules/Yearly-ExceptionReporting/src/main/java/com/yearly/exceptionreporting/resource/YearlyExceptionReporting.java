package com.yearly.exceptionreporting.resource;
...
import com.yearly.exceptionreporting.constants.YearlyExceptionReportingPortletKeys;
...
@Component(property = { 
		"javax.portlet.name=" + YearlyExceptionReportingPortletKeys.YEARLYEXCEPTIONREPORTING,
		"mvc.command.name=" + YearlyExceptionReportingPortletKeys.yearlyExceptionReportDocumentUpload,
		}, 
service = MVCResourceCommand.class)

public class YearlyExceptionReporting implements MVCResourceCommand {
	
	private static Log _log = LogFactoryUtil.getLog(YearlyExceptionReporting.class.getName());
	...
	public JSONObject fileUpload(ThemeDisplay themeDisplay, ResourceRequest resourceRequest) {
		...
		JSONObject resultJsonObject = JSONFactoryUtil.createJSONObject();
			...
			Long docFileEntryId = 0l;
			docFileEntryId = uploadFILETOFOLDER(themeDisplay, resourceRequest, "yearlyExceptionReportDocFile");
			Long pdfFileEntryId = 0l;
			pdfFileEntryId = uploadFILETOFOLDER(themeDisplay, resourceRequest, "yearlyExceptionReportPDFFile");
			
			JSONArray jsonArray = JSONFactoryUtil.createJSONArray();
			jsonArray.put(pdfFileEntryId);
			jsonArray.put(docFileEntryId);
			
			try {
				String remarks = ParamUtil.getString(uploadPortletRequest, "remarks");
				updateReportLog ( userId, pdfFileEntryId, true, reportUploadLogId,  WorkflowConstants.STATUS_PENDING, statusByUserName, serviceContext,remarks, jsonArray.toString());
