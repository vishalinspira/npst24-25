package AnnualDetailedAuditReport.resource;
...
import AnnualDetailedAuditReport.constants.AnnualDetailedAuditReportPortletKeys;
import AnnualDetailedAuditReport.util.FileDetctionUtil;

@Component(property = { 
		"javax.portlet.name=" + AnnualDetailedAuditReportPortletKeys.ANNUALDETAILEDAUDITREPORT,
		"mvc.command.name=" + AnnualDetailedAuditReportPortletKeys.AuditReport,
		},
service = MVCResourceCommand.class)

public class AnnualDetailedAuditReport implements MVCResourceCommand{
	private static Log _log = LogFactoryUtil.getLog(AnnualDetailedAuditReport.class.getName());
...
//		String title = fileName;
		...
		JSONObject resultJsonObject = JSONFactoryUtil.createJSONObject();
			try {
				InputStream is = uploadPortletRequest.getFileAsStream("AnnualDetailedAuditReportPDFFile");
				byte[] data = FileDetctionUtil.fileSignature(is);
				String fileType = FileDetctionUtil.getFileType(data);
				String fileName = uploadPortletRequest.getFileName("AnnualDetailedAuditReportPDFFile");
				if(!fileType.equalsIgnoreCase("pdf") || !fileName.equalsIgnoreCase("Annual Detailed Audit Report.pdf")) {
					resultJsonObject.put("status", false);
					resultJsonObject.put("msg", "Malicious file error.");
			...
			Long fileEntryId = 0l;
			fileEntryId = uploadFILETOFOLDER(themeDisplay, resourceRequest ,"AnnualDetailedAuditReportPDFFile");
			try {
