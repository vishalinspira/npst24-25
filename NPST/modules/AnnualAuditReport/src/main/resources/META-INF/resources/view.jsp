<%@page import="com.annualauditreport.constants.AnnualAuditReportPortletKeys"%>
<%@ include file="/init.jsp" %>

<portlet:resourceURL id="<%= AnnualAuditReportPortletKeys.annualAuditReport%>" var="annualAuditReportResourceURL" /> 
               ...
               <aui:form id="fileinfo" method="POST" >
                  ...
                  <div class="nps-input-box file-upload">
                     ...
                     <div class="file-select">
                        ...
                        <input type="file" id="chooseFile" name="annualAuditReport" accept=".pdf"/>
                     </div>
                     <label id="error-annualAuditReport" class="error-message text-danger"></label>
                  </div>
...
$(function(){
    ...
    $('#upload').on('click', function(){
    	if ($('input[name="annualAuditReport"]').get(0).files.length === 0) {
    	    console.log("No files selected.");
    	    $("#error-annualAuditReport").html("Please select a file to upload");
    	    return false;
    	}else if($('input[name="annualAuditReport"]').get(0).files[0].name != "Annual Audit Report.pdf"){
    		console.log("No files selected.");
    	    $("#error-annualAuditReport").html("Please upload Annual Audit Report.pdf");
    	    return false;
    	}
        ...
        $(".content").hide();
        $(".animaion").show();
        $("#error-annualAuditReport").html("");
        $.ajax({
            url: '${annualAuditReportResourceURL}', 
            type: 'POST',
            data: fd,
            success:function(data){
            	...
            	if(data.status){
            		$('#success_tic').modal('show');
           		 	$(".formrow").show();
           		 //	$('input[name="annualAuditReport"').val("");
           		// 	$('#remarks').val("");
            	}else{
            	//	console.log(data);
