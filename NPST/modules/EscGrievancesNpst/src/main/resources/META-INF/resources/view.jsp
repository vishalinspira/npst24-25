		...
		<portlet:resourceURL id="/escgrievancesnpst/save" var="saveescgrievancesnpstURL"></portlet:resourceURL>
	...
	<div class="nps-page-main nps-upload-report">
        <div class="row">
            <div class="col-lg-7 col-md-10 col-sm-12 col-12">
                <div class="nps-report">
                    <h4>Esc grievances to NPST during Month</h4>
                    <div class="content">
						...
						<input type="hidden" value="${reportMasterId }" name="<portlet:namespace/>reportMasterId" class="reportMasterId"/>
	                        ...
	                        <div class="nps-input-box file-upload">
	                            ...
	                            <div class="file-select">
	                               ...
	                               <input type="file" class="chooseFile" id="chooseFile" name="escgrievancesnpstFile" />                               
	                            </div>
	                            <label id="error-escgrievancesnpstFile" class="error-message text-danger"></label>
	                        </div>
							...
							<div class="col-md-6">
								<div class="form-group">
									<div class="fileUploadInput">
										<input type="file" class="form-control" name="escgrievancesnpstFile" />
										<button type="button" class="btn file-btn">Choose File</button>
									</div>
									<label id="error-escgrievancesnpstFile" class="error-message"></label>
								</div>
							</div>
						</div>
		...
		$(function(){
		    $('#upload').on('click', function(){
		    	if ($('input[name="escgrievancesnpstFile"]').get(0).files.length === 0) {
		    	    console.log("No files selected.");
		    	    $("#error-escgrievancesnpstFile").html("Please select a file to upload");
		    	    return false;
		    	}else if($('input[name="escgrievancesnpstFile"]').get(0).files[0].name != "Month 2021- Esc griev to NPST during Month & pending for more than 60 Daya from griev regn.xlsx"){
		    		console.log("No files selected.");
		    	    $("#error-escgrievancesnpstFile").html("Please upload Month 2021- Esc griev to NPST during Month & pending for more than 60 Daya from griev regn.xlsx");
		    	    return false;
		    	}
		        ...
		        $(".content").hide();
		        $(".animaion").show();
		        $("#error-escgrievancesnpstFile").html("");
		        $.ajax({
		            url: '<%=saveescgrievancesnpstURL %>',  
		            type: 'POST',
		            data: fd,
		            success:function(data){
		            	...
		            	if(data.status){
		            		//var pdfURL = data.pdfURL;
		           		 	...
		           		 	$(".formrow").show();
		           		 	$('input[name="escgrievancesnpstFile"').val("");
		            	}else{
		            		console.log(data);
