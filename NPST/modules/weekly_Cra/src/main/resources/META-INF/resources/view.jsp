...
<portlet:resourceURL id="/weeklycra/save" var="saveweeklycraURL"></portlet:resourceURL>
        ...
        <div class="row">
            <div class="col-lg-7 col-md-10 col-sm-12 col-12">
                <div class="nps-report">
                  <h4>Weekly CRA kfintech NAV Data</h4>
                     <aui:form id="fileinfo" method="POST" >
                        ...
                        <div class="nps-input-box file-upload">
                           ...
                           <div class="file-select">
                              ...
                              <input type="file" id="chooseFile" name="weeklyCraFile" />
                           </div>
                           <label id="error-weeklyCraFile" class="error-message text-danger"></label>
                        </div>
		...
		<div class="row">
		    <div class="col">
		        <div class="page_title">
		            <img src="/o/Liferay-NPS-Theme/images/file-upload.png" alt="Upload Reports" /> Weekly CRA kfintech NAV Data
		        </div>
		    </div>
		</div>
								...
								<div class="col-md-6">
									<div class="form-group">
										<div class="fileUploadInput">
											<input type="file" class="form-control" accept="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" name="weeklyCraFile" />
											<button type="button" class="btn file-btn">Choose File</button>
										</div>
										<label id="error-weeklyCraFile" class="error-message"></label>
									</div>
								</div>
							</div>
...
$(function(){
    $('#upload').on('click', function(){
    	if ($('input[name="weeklyCraFile"]').get(0).files.length === 0) {
    	    console.log("No files selected.");
    	    $("#error-weeklyCraFile").html("Please select a file to upload");
    	    return false;
    	} else if($('input[name="weeklyCraFile"]').get(0).files[0].name != "Weekly kfintech CRA NAV data.xlsx"){
    		console.log("No files selected.");
    	    $("#error-weeklyCraFile").html("Please upload Weekly kfintech CRA NAV data.xlsx");
    	    return false;
    	}
        ...
        $(".content").hide();
        $(".animaion").show();
        $("#error-weeklyCraFile").html("");
        $.ajax({
            url: '<%=saveweeklycraURL %>',  
            type: 'POST',
            data: fd,
