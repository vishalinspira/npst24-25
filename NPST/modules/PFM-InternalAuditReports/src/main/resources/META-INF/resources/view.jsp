<%@page import="com.internalauditreports.constants.PFMInternalAuditReportsPortletKeys"%>
<%@ include file="/init.jsp" %>

<portlet:resourceURL id="<%=PFMInternalAuditReportsPortletKeys.iarDocumentUpload%>" var="iarDocumentResourceURL" /> 
               ...
               <aui:form id="fileinfo" method="POST" >
                  ...
                  <div class="nps-input-box file-upload">
                     ...
                     <div class="file-select">
                        ...
                        <input type="file" id="chooseFile_1" name="iarPDFFile" accept=".pdf"/>
                     </div>
                     <label id="error-iarPDFFile" class="error-message text-danger"></label>
                  </div>
                  <div class="nps-input-box file-upload">
                     <label>Report </label>
                     <div class="file-select">
                        ...
                        <input type="file" id="chooseFile_2" name="iarDocFile" accept=".docx"/>
                     </div>
                     <label id="error-iarDocFile" class="error-message text-danger"></label>
                  </div>
...
$(function(){
    ...
    $('#upload').on('click', function(){
    	if ($('input[name="iarPDFFile"]').get(0).files.length === 0) {
    	    console.log("No files selected.");
    	    $("#error-iarPDFFile").html("Please select a file to upload");
    	    return false;
    	}else if($('input[name="iarPDFFile"]').get(0).files[0].name != "Internal Audit Reports.pdf"){
    		console.log("No files selected.");
    	    $("#error-iarPDFFile").html("Please upload Internal Audit Reports.pdf");
    	    return false;
    	}
    	
    	if ($('input[name="iarDocFile"]').get(0).files.length === 0) {
    	    console.log("No files selected.");
    	    $("#error-iarDocFile").html("Please select a file to upload");
    	    return false;
    	}else if($('input[name="iarDocFile"]').get(0).files[0].name != "Internal Audit Reports.docx"){
    		console.log("No files selected.");
    	    $("#error-iarDocFile").html("Please upload Internal Audit Reports.docx");
    	    return false;
    	}
        ...
        $(".content").hide();
        $(".animaion").show();
        $("#error-iarPDFFile").html("");
        $("#error-iarDocFile").html("");
        $.ajax({
            url: '${iarDocumentResourceURL}', 
            type: 'POST',
            data: fd,
